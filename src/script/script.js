...
let gameFrame = 0; // для добавления событий в игру
...
playerLeft.src = './src/img/cartoon_fish_left_red.png';
const playerRight = new Image();
playerRight.src = './src/img/cartoon_fish_right_red.png';
class Player{
    constructor(){
        this.x = canvas.width;
    ...
    update(){
        ...
        let theta = Math.atan2(dy, dx); // вычесления угла вращения
        this.angle = theta;
        if(mouse.x !== this.x){
            this.x -= dx / 30;//где (/) замедление скорости передвижения
        }
        if(mouse.y !== this.y){
            this.y -= dy / 30;
        }
...
function handleBubless(){
    if(gameFrame % 50 === 0){
        bubblesArray.push(new Bubble())
        // console.log(bubblesArray);
    }
    for(let i = 0; i < bubblesArray.length; i++){
        bubblesArray[i].updata();
        bubblesArray[i].draw();
    }
    for(let i = 0; i < bubblesArray.length; i++){
